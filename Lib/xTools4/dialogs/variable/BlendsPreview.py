...
def drawGlyphTTF(ttfPath, glyphName):
    ...
    f2 = TTFont(ttfPath)
    f2_upm = f2['head'].unitsPerEm
    B = BezierPath()
    B.text(char, font=ttfPath, fontSize=f2_upm)
    DB.drawPath(B)

def getGlyphTTF(ttfPath, glyphName, location):
    char = psname2char(glyphName)
    ttFont = TTFont(ttfPath)
    glyphSet = ttFont.getGlyphSet()
    ttGlyph = glyphSet[glyphName]
    g = RGlyph()
    pen = g.getPen()
    ttGlyph.draw(pen)
    g.width = ttGlyph.width
    return g

def getVarDistance(location, default):
    n = 0
    for key, value in location.items():
        defaultValue = default.get(key)
...
class BlendsPreview:
    ...
    def draw(self, glyphName):
        ...
        if self.compare and self.compareFont:
            unitsPerEm2 = TTFont(self.compareFont)['head'].unitsPerEm
...
class BlendsPreviewDialog:
    ...
    def _updatePreview(self):
        ...
        axesList = []
        for axisItem in axesListItems:
            axis, values = axisItem['axis'], axisItem['values']
            values = [int(v) for v in values.split()]
            axesList.append((axis, values))
        ...
        for i, axisValue1 in enumerate(axis1Values):
            for j, axisValue2 in enumerate(axis2Values):
                for k, axisValue3 in enumerate(axis3Values):
                    ...
                    parametricLocation = getEffectiveLocation(self.designspacePath, blendedLocation)
                    g = instantiateGlyph(self.operator, glyphName, parametricLocation)
                    if not g:
                        continue
            ...
            DB.translate(0, cellSize * len(axis1Values))
...
if __name__ == '__main__':
    ...
    compareFontPath = '/Users/gferreira/hipertipo/fonts/fontbureau/amstelvar-avar2/Fonts/legacy/Amstelvar-Roman[GRAD,XOPQ,XTRA,YOPQ,YTAS,YTDE,YTFI,YTLC,YTUC,wdth,wght,opsz].ttf'
