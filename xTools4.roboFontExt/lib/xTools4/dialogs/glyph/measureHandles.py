...
KEY = f'{hDialog.key}.glyph.measureHandles'
...
class MeasureHandlesController(ezui.WindowController):
    ...
    def started(self):
        registerRepresentationFactory(Glyph, f"{KEY}.handles", measureHandlesFactory)
        registerRepresentationFactory(Glyph, f"{KEY}.segments", measureSegmentsFactory)
        MeasureHandles.controller = self
        registerGlyphEditorSubscriber(MeasureHandles)

    def destroy(self):
        unregisterRepresentationFactory(Glyph, f"{KEY}.handles")
        unregisterRepresentationFactory(Glyph, f"{KEY}.segments")
...
class MeasureHandles(Subscriber):
    ...
    def _drawMeasureHandles(self):
        ...
        if showHandles:
            measurements = self.glyph.getRepresentation(f"{KEY}.handles")
            if measurements:
        ...
        if showLines:
            measurements = self.glyph.getRepresentation(f"{KEY}.segments")
            if measurements:
                positions, lengths, angles = measurements
                captions = lengths if captionMode == 0 else angles
                for i, pos in enumerate(positions):
...
measureHandlesEvent = f"{KEY}.changed"
